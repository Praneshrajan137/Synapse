...
### Deploy Truth Rules (Sprint 14)
- `scripts/deploy/verify_live.py` is the SINGLE deploy-truth oracle. Never add ad-hoc curl checks to workflows — add an assertion to the oracle so all three call sites (cd-gcp post-deploy, live-truth watchdog, `make verify-live`) get it
- Every cd-gcp deploy ends with two BLOCKING verify steps (`--on-vm --probe-decision` and `--external --expect-sha`) and an issue-on-failure step — `verify_claims.py` C47 enforces their presence. NEVER make these steps warn-only
- Every Python image in the cd-gcp build matrix MUST declare its `entry` module for the import-smoke step (C49, lockstep like C27). A new service = matrix entry + `entry` field + `verify_images.sh` row in the same PR
- Every service in `docker-compose.gcp.yml` MUST resolve to a healthcheck — compose-level or Dockerfile `HEALTHCHECK` (C50). A service that overrides `command:` away from the image's server MUST also override the inherited healthcheck (the traffic-generator uses a heartbeat-file check for exactly this reason)
- `.github/workflows/live-truth.yml` is the staleness watchdog (C48): 2x/day it asserts deployed `/version` == main HEAD + container truth, and auto-opens a `sev1` issue. Its cron MUST stay inside the VM's REAL auto-stop window — the attached policy is `synapse-3hr-window` (start 04:30 / stop 07:30 UTC = 10:00–13:00 IST; verified via `gcloud compute operations list` 2026-06-10). The "09:00–23:00 IST" in older ADR-036 notes is STALE — always verify the live policy before touching the cron
- The `traffic-generator` service keeps the live UI fed (one real decision every ~4–5 min, tagged `synthetic-` in `order_id`). Do not remove it without replacing the demo-liveness story
...
- E-S14-04: the VM's REAL auto-stop schedule is whatever resource policy is ATTACHED (`gcloud compute resource-policies list` / `operations list`), not what the docs/terraform examples say. The live policy is `synapse-3hr-window` (04:30–07:30 UTC = 10:00–13:00 IST) while ADR-036 notes said 09:00–23:00 IST — a watchdog cron trusting the docs would have fired on a stopped VM and opened a false sev1 daily
- E-S14-05: right after `up --force-recreate`, 21 containers + 12 image extractions saturate the e2-standard-8 and tight Dockerfile healthchecks (5s timeout, no start_period) flap unhealthy for minutes (disruption-shield was unhealthy at deploy-verify time yet healthy 54s after a calm boot). Post-deploy verification must assert EVENTUAL health within a bounded settle window (`verify_live.py --settle-seconds`), never instant health
- E-S14-07: transient runner-egress flakes (docker.io deadline, sigstore HTTP/2 INTERNAL_ERROR, GHA-cache blob GET) are the DOMINANT CD failure class — one run lost 3 legs to 3 different services in 15 min. Do not add per-step retries reactively; `cd-recovery.yml` re-runs failed jobs once (attempt cap 2) and closes stale `deploy-failure`/`live-truth` issues after a verified-green deploy
- E-S14-06: NEVER download model weights inside a server lifespan — `SentenceTransformer("all-mpnet-base-v2")` in `PlaybookRetriever.__init__` pulled ~420MB from HF Hub on every force-recreated boot (`--force-recreate` wipes the container's HF cache), blocking `/health` for ~11 minutes and failing the C47 gate twice; a calm `docker start` reuses the old container's cache and hides the bug. Init external-index clients FIRST and skip dependent model loads when the index is unconfigured (the embedder was useless without Pinecone anyway). Bake weights into the image or use a cache volume if a startup load is ever genuinely required
...
- **Substance Mandate (Plan v2)** (`plans/c-users-pranesh-downloads-synapse-backe-sequential-treasure.md` — landing): Sprints 11–13 made the *enforcement boundary* honest; this work makes the *substance* honest. The finding: every `agents/*/inference/pipeline.py` returned synthetic features + fallback predictions **unconditionally** (guard-then-ignore), confidence was a constant `0.85` (disabling I-5), and **no agent had a real training loop** (even demand_prophet's `train.py` took zero gradient steps). Deliveries:
  - **Honesty contract (ADR-040, ADR-041)** — `packages/synapse_common/{features,model_registry,provenance,invariants}.py`: `FeatureProvider` + `ModelRegistry` anti-corruption layer over Feast/MLflow (real or honestly-`degraded`, never crashes — I-7), the `Provenance` value object (`model_version`/`feature_source`/`degraded`/`confidence_basis`), and `RuntimeValidator` (re-checks spec postconditions on live output — I-3 at runtime). 19 unit tests.
  - **Substance gate** — `scripts/audit/substance_truth.py` AST-flags the 3 anti-patterns; wired as `verify_claims.py` **C33** (ratchet 10→0) and a blocking `ci.yml` step. All 8 pipelines rewired through the contract: **0 violations, 8/8 clean**. Confidence is now derived per agent (conformal width / waste entropy / posterior spread / elasticity strength).
  - **I-12 wired (C34)** — `TwinKafkaSync` feeds live state into `DivergenceMonitor.update_live_state`; `synapse_digital_twin_kl_divergence` now emits; re-sync fires > 0.1. `update_live_state` previously had no production caller.
  - **API hardened (C35)** — JWT on all decisions/steering endpoints (VIEWER reads, OPS mutations); the hardcoded `synapse_app_2026` DSN credential removed from `decisions.py`, `main.py`, AND `steering.py` (fail-fast). 7 API tests (the suite was empty before).
  - **CI holes closed (Phase 4)** — `digital_twin/` + `api/` added to the gated pytest+coverage run (+floors); `data_fabric`/`ml_pipelines` documented as the remaining ratchet.
  - **Verification** — `make verify-claims` → **30 PASS / 0 FAIL** (C33–C36 added); `docs/quality_gates/definitive_verification.md` honestly separates locally-proven (40 new tests) from CI-gated (torch/ortools training). **Remaining ratchet:** genuine multi-epoch model training (operator/Colab + CI smoke-train), app-layer rate limiting, and binding the 11 `0.0` coverage floors on first CI run.
- **Sprint 14 — Deploy Truth Forever** (PRs #44–#45): a merge to main now either produces a **verified-working live system or a red workflow + auto-opened issue** — no silent third state. Root cause closed: the deploy pipeline could go green while the live system was stale (the `/version` SHA check was a *printed suggestion*) or broken ("Wait for health" probed only nginx; the jsonschema incident shipped 2 crash-looping agents behind a green run; a dev-only dep crashed prod images with CI green). Deliveries: `scripts/deploy/verify_live.py` (the single truth oracle: external mode asserts deployed SHA == merged commit + healthz/topology/agents-no-307/decisions-auth/firehose-WS; on-vm mode asserts every container running+healthy + RestartCount + one decision flowing orchestrator→outbox→Kafka; 9 unit tests); two blocking post-deploy steps + per-SHA-deduped `sev1 deploy-failure` auto-issue in cd-gcp.yml; per-image `entry` import-smoke in the build matrix (runs inside the pushed digest, pre-sign); mlflow/prometheus/grafana healthchecks (21/21 services checked); `.github/workflows/live-truth.yml` watchdog (2x/day inside the VM's real `synapse-3hr-window` policy, skip-if-deploying guard, `sev1 live-truth` auto-issue, max one open) — staleness detection now happens every up-window instead of "whenever a human looks" (historically months); `traffic-generator` compose service (reuses the api-gateway AR image, heartbeat healthcheck) drives one real tagged decision every ~4–5 min so the live UI is never empty and the pipeline is continuously exercised. Gates **C47/C48/C49/C50** + `make verify-live`. `make verify-claims` → 35 PASS / 0 FAIL.
- **Sprint 15 — Living Interface** ([ADR-044](docs/adr/ADR-044-aux-provenance-exposure.md), branch `feat/aux-living-interface`, 5 phases): the backend's intelligence signals now reach the operator's eyes. Finding: `Provenance` died as free-form trace strings (the `payload["provenance"]` dict the consensus read was never written by ANY agent — `degraded_agents` was permanently empty), `audit_consensus` anatomy columns (debate_rounds, pareto_front, execution_confirmations, context_messages, outcome, chain hashes) reached no endpoint, brownout/breaker state was Prometheus-only with NO controllers ever registered (E-S9-04 no-op was permanent), escalations published to a topic the firehose never subscribed, and BOTH the `decision` and `twin` firehose channels were silently dead (strict Zod schemas rejected every live envelope — the "live" tail rendered nothing). Deliveries: **P1 backend truth** — typed `AgentProposal.provenance` attached by all 8 handlers (chain-safe: hashed==stored inside proposals; `make_canonical_row` untouched, byte-pinned); decisions API additive anatomy + `chain_verified` tri-state + `degraded`/`is_synthetic` (SQL jsonpath, prefix owned by `synapse_common/synthetic.py`); chain-hash primitives moved to `synapse_common.audit_chain` (re-exported; mutation gate follows); `escalation` firehose channel (topics.json v1.3.0 registers `api_firehose` on all 9 consumed topics — closes the WS-3 unregistered-consumer gap); `GET /api/v1/system/posture` (brownout per city + breakers; controllers now registered at startup). **P2 Chromatic v1.1.0** (additive, frozen hues untouched): `state.degraded` (chroma-drained honesty marker, INV-CLR-016) + `state.synthetic` (H310, distinctness-probe-tuned) + `factor.agentstate.*` process-state chroma rationing (INV-CLR-017, MR-CLR-004) + urgent/arrive/entrance motion; INV-CLR-015..018; FE mirror byte-pinned against dist. **P3–P5 frontend**: DecisionEnvelope schema (fixes the dead decision channel; envelope enriched with timestamp + lean agents summary), ProvenanceChip/DegradedBanner/SyntheticBadge/ChainIntegrityChip/TwinDivergenceCaveat/ThemeToggle compounds, full decision anatomy + violation severity (never count-only) + override history, process-state grammar on CouncilStrip/AgentDetail (acting/waiting/interrupted derived honestly — thinking/debating never fabricated), urgency-as-frequency motion (escalation beacon at half the ambient breath), 14 chromatic-allow raw literals migrated to `rgb(var(--syn-*))`. FE-INV-034..040. FE 244 tests / registry 40 invariants / entry 131kB gz; `verify_claims` 44 PASS / 0 FAIL.
- **Sprint 16 — Obsidian Control Room** ([ADR-045](docs/adr/ADR-045-obsidian-visual-identity.md), branch `sprint16/obsidian`, 5 phases): the visual reinvention + no-login boot. Diagnosis: the fonts never loaded (config named faces that existed nowhere — everyone saw system-ui), the FE ran a parallel slate-Tailwind token fork that ignored the governed OKLCH system (tier was emerald/sky/amber/rose, violating INV-CLR-006), and every panel was border-grid chrome. Deliveries: **P1** boot-authenticated console (`useBootAutoLogin` signs in the demo admin silently; honest fallback to /login on failure; `VITE_AUTO_LOGIN=false` restores the wall). **P2** Chromatic v1.2.0 cold-void neutral retune (H265, chroma rises with depth, surface steps sized for borderless elevation; APCA gates rejected three candidate text values — shipped 0.972/0.790/0.700 are gate-minimal; 58/58 spec tests). **P3** FE foundation: self-hosted Inter Variable + Space Grotesk + JetBrains Mono (OFL, `public/fonts/FONTS.md`), tokens.css transcribed from dist rgb255 (tier = the H258 ramp at last), borderless `.syn-card` depth (per-theme: dark borderless / light hairline / hc hard borders), brand ambient glow, MapLibre/deck mirrors, and the **drift-killer** (chromatics.test.ts pins 22 CSS mirrors + viz literals against dist); also fixed the no-flash theme script reading a key that never existed. **P4** BrandMark pulse-ring glyph, Shell nav with brand underline rail, PageHeader compound, ConfidenceChip on the CONTINUOUS gate-anchored scale with VSUP (legacy 3-stop ramp quantised at the wrong 0.9 gate), display-numeral KPITile. **P5** Mission Control hero recomposition (.syn-hero Cortex, 380px firehose rail), PageHeader across all surfaces, AuditVault sticky/dense table. FE 258 tests / 40 invariants / entry 131.8kB gz; design system 58/58.
