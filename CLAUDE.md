...
- **Sprint 10**: GCP-first deployment — Terraform module (12 .tf files, +3,179 LOC) on branch `feat/gcp-primary-deployment` (`cc96b54`). Workload Identity Federation, Artifact Registry, Cloud Armor, Secret Manager, GCS state backend, `docker-compose.gcp.yml`, `.github/workflows/cd-gcp.yml`, ADR-036 (single-VM not GKE) + ADR-037 (dual-deployment cost honesty). **Status: pushed to origin, merges cleanly into `main` with zero conflicts, awaiting PR merge** (WS-1 of the verification-rigor plan).
- **Sprint 11 — Verification Rigor** (in flight via `plans/i-have-finished-most-nifty-sifakis.md`): every load-bearing CLAUDE.md claim becomes a mechanical CI gate. Deliveries by workstream:
  - **WS-0**: `docs/state/CURRENT.md` truth ledger + `scripts/audit/verify_claims.py` (one check per former drift point).
  - **WS-1**: GCP branch merge artefacts (`terraform-validate.yml`, `.env.cloud.{gcp,oracle}.example`, `docs/deploy/README.md`).
  ...
  - **WS-11**: this entry + README + ADR-038/039 + `make verify-claims` count posted to PR.
