...
- **Sprint 9**: Supply chain + data lifecycle + production topology — CycloneDX SBOMs, cosign keyless signing, uv hash-pinned deps, audit chained-hash + verifier CLI + daily anchor, gated security scans, CVE-budget enforcement, SOPS secrets skeleton, audit archival (MinIO), DPDPA cascade helper, PII redaction structlog processor, KV-cache PII guard, Feast compaction, APScheduler jobs, brownout-budget integration, reward cutover (8/8 agents have reward_config.py), 200 golden traces, live LLM-judge behind flag, Helm chart skeleton (api + orchestrator + demand_prophet) + Linkerd/KEDA/Flagger CRDs + DR runbooks, ADRs 033-035, 4 remaining test_reward_safety files, KV-cache 0.70 floor, tier-routing 80% floor
