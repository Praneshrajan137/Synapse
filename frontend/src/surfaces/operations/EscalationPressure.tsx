...
export function EscalationPressure({ city }: { city: City }) {
  ...
  const d = q.data;
  const mix = d?.override_actions ?? { approved: 0, rejected: 0, modified: 0, none: 0 };
  const mixTotal = Math.max(1, mix.approved + mix.rejected + mix.modified + mix.none);
  ...
  return (
    <section className="syn-card space-y-3 p-4" aria-label="Escalation pressure">
      ...
      <div className="grid grid-cols-3 gap-3">
        <KPITile label="Escalations" value={d?.total ?? 0} />
        <KPITile
          label="Pending"
          value={d?.pending ?? 0}
      ...
      <div className="space-y-1">
        <div className="text-2xs uppercase tracking-[0.14em] text-ink-subtle">
          Override mix · {d?.window_hours ?? 24}h
        </div>
