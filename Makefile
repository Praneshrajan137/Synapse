...
# ============================================================================
# GCP DEPLOYMENT — mirror of the deploy-oracle-* block above.
...
GCP_COMPOSE := docker compose -f docker-compose.gcp.yml --env-file .env.gcp
...
deploy-gcp-setup: ## Bootstrap GCP VM (Docker, Ollama, certbot, swap, backup cron)
...
deploy-gcp-push: ## Push compose + env to GCP VM and (re)start services
	...
	ssh -i $(GCP_KEY) $(GCP_USER)@$(GCP_IP) 'chmod +x ~/synapse/backup_to_gcs.sh'
	@echo "Building images + starting services (this can take 10-15 min on first run)..."
	ssh -i $(GCP_KEY) $(GCP_USER)@$(GCP_IP) 'cd ~/synapse/docker && $(GCP_COMPOSE) up -d --build'

deploy-gcp-verify: ## Verify GCP deployment health
	@test -n "$(GCP_IP)" || (echo "ERROR: GCP_IP not set" && exit 1)
	@echo "=== docker compose ps ==="
	ssh -i $(GCP_KEY) $(GCP_USER)@$(GCP_IP) 'cd ~/synapse/docker && $(GCP_COMPOSE) ps'
	@echo ""
	@echo "=== Endpoint health ==="
	@curl -sf http://$(GCP_IP):3000/api/health | python -c "import sys,json; d=json.load(sys.stdin); print(f'Grafana:    {d.get(\"database\",\"unknown\")}')" 2>/dev/null || echo "Grafana:    not reachable (check firewall + admin_ip_cidr)"
	@curl -sf http://$(GCP_IP):9090/-/healthy >/dev/null && echo "Prometheus: OK" || echo "Prometheus: not reachable"
	@curl -sf http://$(GCP_IP):5000/api/2.0/mlflow/experiments/search?max_results=1 >/dev/null && echo "MLflow:     OK" || echo "MLflow:     not reachable"
	@curl -sf http://$(GCP_IP)/healthz >/dev/null && echo "Frontend:   OK (via nginx)" || echo "Frontend:   not reachable"
	@echo ""
	@echo "============================================="
	@echo "  GCP deployment verified"
	@echo "  Frontend  : http://$(GCP_IP)/"
	@echo "  Grafana   : http://$(GCP_IP):3000"
	@echo "  Prometheus: http://$(GCP_IP):9090"
	@echo "============================================="
...
deploy-gcp-destroy: ## Tear down GCP infra (terraform destroy — also wipes the backup bucket)
	@echo "WARNING: this destroys the VM, disk, IP, and backup bucket (with versioning off, backups are GONE)."
...
gcp-shell: ## Interactive SSH session on the GCP VM
	@test -n "$(GCP_IP)" || (echo "ERROR: GCP_IP not set" && exit 1)
	ssh -i $(GCP_KEY) $(GCP_USER)@$(GCP_IP)
