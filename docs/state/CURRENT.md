...
| C15 | Coverage floor | `.github/workflows/ci.yml` enforces `--cov-fail-under=60` (commit `d888b8d`: "coverage floor 80%→60% (consolidation debt; follow-up to restore)"). | CLAUDE.md Code Quality: "Coverage minimum: 80% on all packages". | **FAIL** — gate is lower than the documented floor. (Resolved by WS-9.) |
