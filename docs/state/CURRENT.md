...
| Tier 4 (digital-twin, C7) | ✅ **wired** | `_phase_twin_simulate` invokes the twin's A2A `simulate` (Monte-Carlo what-if) on Tier-4 and records the result in the audit context — the production caller the twin never had. Degrades honestly if the twin is unreachable. Gate **C7** asserts the caller exists. |
