...
- **Total mechanical checks:** 48 (registered in `scripts/audit/verify_claims.py`; Sprint 20 added **C55** outbox ORM↔DDL truth and removed **C54** — the AGENTS.md/`.agents` mirror gate — when the cross-tool mirror was dropped per operator preference)
- **PASS:** 47 locally — incl. C7 (orchestrator↔twin + input provenance), C37 (6/8 real-loop/analytical fits), **C39 (8/8 wired — every agent loads a model via ModelRegistry)**, C42 (3/8 torch-free runtime probes PASS locally), **C55 (audit_outbox DDL == ORM — fresh-DB decision logging restored)**
- **FAIL:** 0
- **PARTIAL:** 0
