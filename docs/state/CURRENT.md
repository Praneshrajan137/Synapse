...
| C50 | Every GCP compose service resolves to a healthcheck | All 20 services in `docker-compose.gcp.yml` carry a healthcheck — compose-level (kafka/redis/neo4j/postgres/orchestrator/api/frontend/nginx + newly added mlflow/prometheus/grafana) or Dockerfile `HEALTHCHECK` (8 agents, digital-twin). Verified by `verify_claims.py::check_compose_health_complete`. | Sprint 14. C47's "all containers healthy" assertion is only total if every container actually carries a healthcheck. | **PASS** — no service can run silently un-monitored. |
